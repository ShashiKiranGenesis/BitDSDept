...
// Importing MySQL transaction files for fpc table operations
const getFPCByVtuIdAndYear = require("../../../database/tables/f_p_c/getFPCByVTUIDandYear");
const deleteFPCById = require("../../../database/tables/f_p_c/deleteFPCById");
const insertSingleFPC = require("../../../database/tables/f_p_c/insertSingleFPC");
const insertMultipleFPC = require("../../../database/tables/f_p_c/insertMultipleFPC");
const getFPCById = require("../../../database/tables/f_p_c/getFPCById");
...
router.route("/:vtu_id")
    ...
    .get(async function (req, res) {
        ...
        if (vtu_id !== userId && !isAuthorized(req, 5))
            ...
            result = await getFPCByVtuIdAndYear(arguments);
    ...
    .post(async function (req, res) {
        ...
        arguments = { vtu_id, date_of_event, title_with_page, description, peer_review_desc, no_of_co_authors, main_author, api_score };
        ...
        if (vtu_id !== userId && !isAuthorized(req, 5))
            ...
            result = await insertSingleFPC(arguments);
    ...
    .delete(async function (req, res) {
        ...
        if (isAuthorized(req, 5))
            result = await deleteFPCById({ id });

        // If not, check if the user is deleting his own row
        else {
            const rowData = await getFPCById({ id });
            if (rowData?.error)
...
router.post("/m/:vtu_id", async function (req, res) {
    ...
    if (vtu_id !== userId && !isAuthorized(req, 5))
        ...
        result = await insertMultipleFPC(
