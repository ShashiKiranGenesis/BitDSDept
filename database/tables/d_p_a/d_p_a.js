// Imports from server files
const connect = require("../../connect");


// The original developemnt_programme_attended table schema
const createTableQuery = `
    CREATE TABLE development_programme_attended (
        id INT AUTO_INCREMENT PRIMARY KEY,
        vtu_id VARCHAR(15) NOT NULL,
        date_of_event DATE NOT NULL,
        description VARCHAR(100),
        duration TINYINT NOT NULL,
        organized_by VARCHAR(50) NOT NULL,
        api_score TINYINT NOT NULL,

        FOREIGN KEY (vtu_id) REFERENCES employees(vtu_id),

        CONSTRAINT unique_row_dpa UNIQUE(
            vtu_id,
            date_of_event,
            duration,
            organized_by, 
            api_score
        )
    );
`;
// More details on The Table
/*
duration is in days
*/

// For any Altering in the future
const alterTableQuery = `
	
`;

(async function () {
    let connection;
    try {
        connection = await connect();

        /* Load your Query here */
        await connection.query(createTableQuery);

        console.log("PASS    CODE: DB_TC_23");
    } catch (error) {
        console.error("ERROR    CODE: DB_TC_23");
        console.error(error.message);
    } finally {
        connection.end();
    }
})();